...
//    private EntityManager entityManager;
  ...
  public List<PlaceInfoDTO> getDormList(int areaCode, List<Integer> sigunguCodeList) {
    ...
    String url_1 = "?MobileOS=ETC" + "&MobileApp=MSGS" + "&pageNo=1" + "&numOfRows=50" + //50개 출력됨.
        "&arrange=Q" + "&contentTypeId=32" + //숙박
        "&areaCode=" + areaCode;
    ...
    for (int sigunguCode : sigunguCodeList) {
      ...
      String response = wc.get().uri(url, decodingKey).retrieve()
                          .bodyToMono(String.class) //MonoFlatMap형 리턴함
                          .block();

      JSONObject items = XML.toJSONObject(response).getJSONObject("response")
      ...
      for (int i = 0; i < item.length(); i++) {
        ...
        String modifiedTitle =
            (title.indexOf("[") != -1 && title.indexOf("]") != -1) ? title.substring(0,
                title.indexOf("[")) : title;
  ...
  public List<PlaceInfoDTO> getPlaceList(int areaCode, List<Integer> sigunguCodeList) {
    ...
    String url_1 = "?MobileOS=ETC" + "&MobileApp=MSGS" + "&pageNo=1" + "&numOfRows=30" + //30개 출력됨.
        "&arrange=Q" + "&areaCode=" + areaCode;

    //시군구 코드 Ex[1, 5, 7] 반복문
    for (int sigunguCode : sigunguCodeList) {
      for (int contentTypeId : contentTypeIds) { //12=관광지, 39=음식점

        String url_2 = sigunguCode == 0 ? "&contentTypeId=" + contentTypeId //관광지 or 음식점
  ...
  @Override
  public Boolean saveSchedule(List<String> dateList, Map<Integer, List<PlanBlockDTO>> planList,
      String cityName) {
    return null;
  }
...
//    planList(trip 페이지에서 수정한 일정) 업데이트
  public Boolean updateSchedule(List<String> dateList, Map<Integer, List<PlanBlockDTO>> planList,
      String scheduleId) {

    try {

      /*TRIP_SCHEDULE 업데이트 -> mod_date(수정 시간) 컬럼 추가하기 위함*/
      Optional<Trip> tripSchedule = scheduleDAO.findById(
          Integer.valueOf(scheduleId)); // id 이용해서 Trip 엔티티 가져오기 */
